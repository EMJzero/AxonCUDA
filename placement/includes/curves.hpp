...
inline std::vector<coords> hilbertPlacement(uint32_t nodes, uint32_t width, uint32_t height) {
    ...
    dimensions region = squareishRegion(nodes, width, height, 2);
    width = region.w;
    height = region.h;
...
inline std::vector<coords> snakePlacement(uint32_t nodes, uint32_t width, uint32_t height, bool vertical_first = true) {
    std::vector<coords> result;

    if (nodes == 0)
        return result;

    dimensions region = squareishRegion(nodes, width, height);
...
inline std::vector<coords> zorderPlacement(uint32_t nodes, uint32_t width, uint32_t height) {
    std::vector<coords> result;
