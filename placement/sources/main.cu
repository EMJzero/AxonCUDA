...
int main(int argc, char** argv) {
    ...
    auto place_routine = [&]<Topology T>() -> int {
        ...
        // best placement
        Coord *d_best_placement = nullptr; // placement[node idx] -> x and y placement coordinates of node
        uint32_t *d_best_inv_placement = nullptr; // inv_placement[flat coord idx] -> idx of the node occupying such place, or UINT32_MAX
        // |
        // best results
        float best_whops = FLT_MAX; // total weight of hedge hops in the current best solution
        // |
        // 1D to 2D map
        Coord *d_1dto2d_placement = nullptr; // 1dto2d_placement[node idx] -> x and y coordinates for the node's sequence mapping from 1D to 2D
        ...
        CUDA_CHECK(cudaMalloc(&d_best_placement, num_nodes * sizeof(Coord)));
        CUDA_CHECK(cudaMalloc(&d_best_inv_placement, topo.extent().volume() * sizeof(uint32_t)));
        CUDA_CHECK(cudaMalloc(&d_1dto2d_placement, num_nodes * sizeof(Coord)));
        ...
        CUDA_CHECK(cudaMemcpyToSymbol(c_topo<T>, &topo, sizeof(T), 0, cudaMemcpyHostToDevice));
        ...
        CUDA_CHECK(cudaMemcpy(d_1dto2d_placement, h_1dto2d_placement.data(), num_nodes * sizeof(Coord), cudaMemcpyHostToDevice));
        thrust::device_ptr<const Coord> t_1dto2d_placement(d_1dto2d_placement);
        ...
        CUDA_CHECK(cudaMemcpy(h_placement.data(), d_best_placement, num_nodes * sizeof(Coord), cudaMemcpyDeviceToHost));
        ...
        LOG(cfg) {
            ...
            for (uint32_t i = 0; i < std::min<uint32_t>(num_nodes, VERBOSE_LENGTH); ++i) {
                Coord place = h_placement[i];
                std::cout << "  node " << i << " -> " << place.toString() << "\n";
            }
            std::vector<uint32_t> final_inv_place_tmp(topo.extent().volume());
            CUDA_CHECK(cudaMemcpy(final_inv_place_tmp.data(), d_best_inv_placement, topo.extent().volume() * sizeof(uint32_t), cudaMemcpyDeviceToHost));
            std::cout << "Final inverse placement:\n";
            uint32_t neigh_rotator = 0u;
            Coord place;
            place.setAll(0);
            for (uint32_t i = 0; i < std::min<uint32_t>(topo.extent().volume(), VERBOSE_LENGTH); ++i) {
                std::cout << "  plc " << place.toString() << " -> " << final_inv_place_tmp[topo.flattenedIdx(place)] << "\n";
                while (!topo.contains(topo.neighbor(place, neigh_rotator)))
                    neigh_rotator = (neigh_rotator + 1) % T::neighborsCount();
                place = topo.neighbor(place, neigh_rotator);
                neigh_rotator = (neigh_rotator + 1) % T::neighborsCount(); // keep advancing so non-mesh topologies (e.g. Arbitrary) don't just bounce between 2 nodes
