...
extern __global__ void hyperedge_overlap_kernel( const uint32_t* hedge_offsets, const uint32_t* hedges, float* averages, uint32_t avg_overlaps);
...
int main(int argc, char** argv) {
    ...
    uint32_t avg_overlaps = static_cast<uint32_t>(hg.hedges().size());
    std::vector<uint32_t> hedge_offsets;
    hedge_offsets.reserve(avg_overlaps + 1);

    // prepare hedge offsets
    for (uint32_t i = 0; i < avg_overlaps; ++i)
    ...
    cudaMalloc(&d_avgs, avg_overlaps * sizeof(float));
    ...
    // kernel launch
    int threads_per_block = 128; // multiple of 32
    int num_warps = (avg_overlaps + 1 - 1) / 1; // 1 warp per hyperedge
    int blocks = (num_warps * WARP_SIZE + threads_per_block - 1) / threads_per_block;

    std::cout << "Running kernel...\n";
    hyperedge_overlap_kernel<<<blocks, threads_per_block>>>(d_offsets, d_hedges, d_avgs, avg_overlaps);
    cudaDeviceSynchronize();

    // move back results
    std::vector<float> avgs(avg_overlaps);
    cudaMemcpy(avgs.data(), d_avgs, avg_overlaps*sizeof(float), cudaMemcpyDeviceToHost);
