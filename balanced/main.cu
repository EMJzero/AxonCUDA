...
void printHelp() {
    std::cout <<
        ...
        "  prog -h\n\n"
        "Options:\n"
        "  -r <file>   Reload hypergraph from file ('.hgr' files will interpreted in hMetis' format, others as our binary format)\n"
        "  -s <file>   Save partitioned hypergraph to file\n"
        "  -k <uint>   Number of partitions to construct\n"
        "  -e <float>  Partition balance tolerance (epsilon)\n"
        "  -h          Show this help\n";
}
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-h") { printHelp(); return 0; }
            ...
            if (i + 1 >= argc) { std::cerr << "Error: -k requires an integer number of partitions\n"; return 1; }
            unsigned long value = std::stoul(argv[++i]);
            if (value > std::numeric_limits<uint32_t>::max()) throw std::out_of_range("Error, k too large");
            k_partitions = static_cast<uint32_t>(value);
        } else if (arg == "-e") {
            if (i + 1 >= argc) { std::cerr << "Error: -e requires a float epsilon value\n"; return 1; }
            try { epsilon = std::stof(argv[++i]); }
            catch (const std::exception&) { std::cerr << "Error: invalid value for -e (must be float)\n"; return 1; }
        } else { std::cerr << "Unknown option: " << arg << "\n"; return 1; }
    }
    if (epsilon >= k_partitions - 1) std::cerr << "WARNING, by violating 'epsilon < k - 1' one partition could contain the whole hypergraph !!\n";

    // load constraints
    if (k_partitions == 0u) {
        std::cerr << "WARNING, value provided for the number of partitions k (-k), defaulting to 1024 !!\n";
        k_partitions = 1024u;
    }
    if (epsilon == -1.0f) {
        std::cerr << "WARNING, value provided for epsilon (-e), defaulting to 0.1 !!\n";
        epsilon = 1024u;
    }

    std::cout << "Using constraints:\n";
    std::cout << "  Partitions k:  " << k_partitions << "\n";
    std::cout << "  Epsilon:       " << std::fixed << std::setprecision(5) << epsilon << "\n";
    ...
    if (!load_path.empty()) {
        ...
        std::cout << "  Total hyperedges weight: " << std::fixed << std::setprecision(3) << hg.totalSpikeFrequency() << "\n";
    } else {
    ...
    // HP: no duplicates per hedge, no self-cycles (keep the src only)
    ...
    // HP: no duplicates in the final touching set
    // NOTE: the HyperGraph class keeps the inbound/outbound distinction, but we don't use it here
    ...
    // constraints
    // NOTE: the ceiling is a slight relaxation...
    const uint32_t h_max_nodes_per_part = (uint32_t)std::ceil((1 + epsilon) * ((float)num_nodes / k_partitions));
    const uint32_t target_parts = k_partitions;
    ...
    std::function<std::tuple<uint32_t, uint32_t*>(const uint32_t, const uint32_t, uint32_t*&, dim_t*&, dim_t, uint32_t*&, dim_t*&, dim_t, uint32_t*&)> coarsen_refine_uncoarsen = [&](
        ...
        *     when applying the sequence of size (balancing) events up to its rank
        ...
        */
        ...
        // TODO: ensure this produces "k" partitions exactly!
        if (new_num_nodes <= target_parts || new_num_nodes == curr_num_nodes) {
            ...
            if (new_num_nodes <= target_parts) {
                ...
                std::cout << "FAILED TO COARSEN FURTHER at level " << level_idx << ", remaining nodes=" << curr_num_nodes << " number of partitions=" << new_num_nodes << " required partitions=" << target_parts << "\n";
        ...
        masked_value_functor_small masked_scores { thrust::raw_pointer_cast(t_scores), thrust::raw_pointer_cast(t_valid_moves) };
    ...
    // check the result's validity
    bool valid = num_partitions <= target_parts;
    if (!valid) std::cout << "INVALID PARTITIONING, more initial partitions (" << num_partitions << ") than 'k' (" << target_parts << ")\n";
    ...
    if (valid) {
        ...
        if (partitions_count > target_parts) { std::cout << "INVALID PARTITIONING, more final partitions (" << partitions_count << ") than 'k' (" << target_parts << ")\n"; valid = false;}

        if (valid) {
            // check the nodes per partition constraint
            for (const auto& kv : partitions_counter) {
                if (kv.second > h_max_nodes_per_part) {
                    std::cout << "INVALID PARTITIONING: more node per partition (" << kv.second << ") than epsilon allows (" << h_max_nodes_per_part << ")\n";
                    valid = false;
                    break;
                }
            }
    ...
    if (valid) {
        ...
        std::cout << "  Total cut cost: " << partitioned_hg.totalSpikeFrequency() << "\n";
